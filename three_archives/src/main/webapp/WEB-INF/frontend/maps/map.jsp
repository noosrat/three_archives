...
function initialize() {
	...
	<%FedoraDigitalObject picture = (FedoraDigitalObject) request.getAttribute("digitalObject");%>
	  console.log('<%=picture%>');
	  
	  if (<%request.getAttribute("digitalObject");%>!=null) {
		   	
		
	  
	  google.maps.event.addListener(map, 'click', function(e) {
		  var newimage = {url: '<%=picture.getDatastreams().get("IMG").getContent()%>', size: null, origin: null, anchor: null, scaledSize: new google.maps.Size(40, 40)}
	...
	for(FedoraDigitalObject object: coord){
		...
		var image1 = {url:'<%=picture.getDatastreams().get("IMG").getContent()%>', size: null, origin: null, anchor: null, scaledSize: new google.maps.Size(40, 40)}
