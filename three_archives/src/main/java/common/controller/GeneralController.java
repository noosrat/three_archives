...
public class GeneralController implements Controller {
	...
	private void storeAllArchivePropertiesWithinSession(HttpServletRequest request) {
		...
		System.out.println("SERVICES FOR ARCHIVE " );
		HashMap<String,String> s = (HashMap<String,String>)(session.getAttribute("SERVICES"));
		for (String serv: s.keySet()){
	...
	private void loadArchiveProperties() {
		...
		if (directory != null) {
			...
			System.out.println("IS THIS EXISTING " + directory.exists());
		...
		System.out.println("FIN LOAD ARCHIVE PROPERTIES " + ARCHIVES.size());
	}
