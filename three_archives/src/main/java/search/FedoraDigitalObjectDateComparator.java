...
public class FedoraDigitalObjectDateComparator implements Comparator<FedoraDigitalObject> {
	...
	public int compare(FedoraDigitalObject o1, FedoraDigitalObject o2) {
		...
		// we need to extract the dublin core datastream and compare the title
		// of these
		...
		Integer year2 =null;
		try{
		year1 = Integer.parseInt(dco1.getDublinCoreMetadata().get(DublinCore.DATE.name()));
		year2 = Integer.parseInt(dco2.getDublinCoreMetadata().get(DublinCore.DATE.name()));
		}
		catch(NumberFormatException ex){
			System.out.println("COULD NOT SORT RESULTS.  INCORRECT DATE FORMAT");
		}
